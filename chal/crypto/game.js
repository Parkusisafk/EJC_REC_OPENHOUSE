...
for (let c of text){
    ...
    //fun facts
    if (['{','}'].includes(c)){
        char.title="These curly braces are commonly used to enclose functions or as dictionaries/sets in python"
    }
    if (['(',')'].includes(c)){
        char.title="These round braces are commonly used to call a function"
    }
    if (['<','>'].includes(c)){
        char.title="These right angled brackets are used in C++ to define the type of element stored in a data structure"
    }
    if (c=='.'){
        char.title="You can see these periods used for traversing directories in bash"
    }
    if (c==','){
        char.title="You can typically see these commas used to separate elements in an array"
    }
    if (c=='?'){
        char.title="They are question marks to you, but for some (C++, Javascript), they mean if. In bash, it is a wildcard"
    }
    if (c=='#'){
        char.title="You can see these hashes used for commenting in Python"
    }
    if (c=='-'){
        char.title="Used to indicate flags in commands used in the terminal"
    }
    if (c==' '){
        char.title="These are just some empty spaces for you, but you computer reads it as U+0020"
    }
    if (c==':'){
        char.title="This colon is the else to the ?"
    }
    if (c=='\n'){
        char.title="YOU FOUND AN ESCAPE CHARACTER! 🥳"
    }
...
deletor.id="deletor"
deletor.innerHTML='❌ Ca<u title="(Hint Hint)">N</u>cel' //Hint #1
...
document.getElementById("answer").addEventListener("keydown", (e)=>{
    if (e.key=="Enter"){
        console.log(btoa(document.getElementById("answer").value.toUpperCase()))
    if(atob(["RUpDUkVDe0lUV0FTTk9UU09IQVJEV0FaWklUI0NSWVBUT30=","RUpDUkVDeyNDUllQVE9HUkFQSFkjQ09NUExJQU5DRX0=","RUpDUkVDeyNTVE9SWVRJTUVYI0NSWVBUT30=","RUpDUkVDeyNDUllQVE9MRVJUSFdSRkRHWX0="][textnum])!=document.getElementById("answer").value.toUpperCase()){
    ...
    body: JSON.stringify({
      ...
      score: dt, // OK, I'll use time elapsed in milliseconds
